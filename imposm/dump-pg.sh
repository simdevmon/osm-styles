...
mkdir -p work
chown $USERNAME: work
docker run --name "$PG_CONTAINER" -p $PG_PORT:5432 -e POSTGRES_USER="docker" -e POSTGRES_PASS="docker" -d  -v `pwd`/work:/tmp/work -t kartoza/postgis:$PG_IMAGE_VERSION
...
docker exec -it -u root -e PGPASSWORD=docker $PG_CONTAINER pg_dump -v -x -U docker -h 127.0.0.1 gis -f /tmp/work/gis.backup -F c
